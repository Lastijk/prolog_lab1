...
% общий предок lca(Person1, Person2, Ancestor)
lca(X, Y, Anc) :-
...
% степень родства kinship_degree(Person1, Person2, Degree)
kinship_degree(A, B, Degree) :-
    bfs([[A]], B, RevPath),
...
% проверка логичности возрастов: родитель старше ребёнка минимум на 16 лет
ages_ok :-
    \+ bad_age.
...
% сравнение дат: date_before(D1, D2) истинно, если D1 < D2 (строго раньше)
...
% если ребёнок "старше" родителя, разница будет отрицательной
